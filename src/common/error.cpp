...
err_panic::err_panic(const string &new_msg) : msg(new_msg) { }
err_panic::err_panic(const char *new_msg) : msg(new_msg) { }
err_panic::~err_panic() { }
void err_panic::show_to(ostream &out) const {
    out << "PANIC: " << msg;
}

err_tbd::err_tbd(const string &new_msg) : err_panic(new_msg) { }
err_tbd::err_tbd(const char *new_msg) : err_panic(new_msg) { }
err_tbd::~err_tbd() { }
void err_tbd::show_to(ostream &out) const {
    out << "TBD: " << msg;
}

err_out_of_mem::err_out_of_mem() : err_panic("out of memory") { }
err_out_of_mem::~err_out_of_mem() { }

err_thread_spawn::err_thread_spawn(const string &s)
    : err_panic("thread creation failed: " + s) { }
err_thread_spawn::~err_thread_spawn() { }

err_bad_mem_img::err_bad_mem_img() { }
err_bad_mem_img::~err_bad_mem_img() { }
void err_bad_mem_img::show_to(ostream &out) const {
    out << "failed to read memory image";
}

err_bad_reg::err_bad_reg(unsigned new_reg_no) : reg_no(new_reg_no) { }
err_bad_reg::~err_bad_reg() { }
void err_bad_reg::show_to(ostream &out) const {
    out << "bad register number: " << reg_no;
}

err_runtime_exc::err_runtime_exc() { }
err_runtime_exc::~err_runtime_exc() { }

exc_bad_instr::exc_bad_instr(const uint32_t new_encoding)
    : encoding(new_encoding) { }
exc_bad_instr::~exc_bad_instr() { }
void exc_bad_instr::show_to(ostream &out) const {
    out << "unknown instruction encoding: " << encoding;
}

exc_reserved_hw_reg::exc_reserved_hw_reg(unsigned new_reg_no)
    : reg_no(new_reg_no) { }
exc_reserved_hw_reg::~exc_reserved_hw_reg() { }
...
exc_int_overflow::exc_int_overflow() { }
exc_int_overflow::~exc_int_overflow() { }
void exc_int_overflow::show_to(ostream &out) const {
    out << "integer overflow";
}

exc_addr_align::exc_addr_align() { }
exc_addr_align::~exc_addr_align() { }
void exc_addr_align::show_to(ostream &out) const {
    out << "integer overflow";
}

exc_bad_syscall::exc_bad_syscall(uint32_t n) : syscall_no(n) { }
exc_bad_syscall::~exc_bad_syscall() { }
void exc_bad_syscall::show_to(ostream &out) const {
    out << "bad system call number: " << syscall_no;
}

exc_syscall_exit::exc_syscall_exit(uint32_t n) : exit_code(n) { }
exc_syscall_exit::~exc_syscall_exit() { }
void exc_syscall_exit::show_to(ostream &out) const {
    out << "exit(" << exit_code << ")";
}

exc_no_network::exc_no_network(uint32_t new_cpu) : cpu(new_cpu) { }
exc_no_network::~exc_no_network() { }
...
exc_new_flow_mid_dma::~exc_new_flow_mid_dma() { }
...
exc_bad_queue::exc_bad_queue(uint32_t new_node, uint32_t new_queue)
    : node(new_node), queue(new_queue) { }
exc_bad_queue::~exc_bad_queue() { }
void exc_bad_queue::show_to(ostream &out) const {
    out << "queue " << setfill('0') << hex
        << setw(2) << node << ":" << setw(2) << queue << " does not exist";
}

err_empty_queue::err_empty_queue(uint32_t new_node, uint32_t new_queue)
    : node(new_node), queue(new_queue) { }
err_empty_queue::~err_empty_queue() { }
...
err_duplicate_queue::err_duplicate_queue(uint32_t new_node,
                                         uint32_t new_queue)
    : node(new_node), queue(new_queue) { }
err_duplicate_queue::~err_duplicate_queue() { }
...
err_too_many_bridge_queues::~err_too_many_bridge_queues() { }
...
err_claimed_queue::err_claimed_queue(uint32_t new_node,
                                     uint32_t new_queue)
    : node(new_node), queue(new_queue) { }
err_claimed_queue::~err_claimed_queue() { }
...
err_duplicate_bridge_queue(uint32_t new_node, uint32_t new_queue)
    : node(new_node), queue(new_queue) { }
err_duplicate_bridge_queue::~err_duplicate_bridge_queue() { }
...
err_duplicate_link_queue::~err_duplicate_link_queue() { }
...
err_duplicate_ingress::~err_duplicate_ingress() { }
...
err_duplicate_egress::~err_duplicate_egress() { }
...
err_bad_next_hop::~err_bad_next_hop() { }
...
err_bad_next_hop_queue::~err_bad_next_hop_queue() { }
...
err_bad_neighbor::err_bad_neighbor(uint32_t new_node, uint32_t new_nbr)
  : node(new_node), neighbor(new_nbr) { }
err_bad_neighbor::~err_bad_neighbor() { }
void err_bad_neighbor::show_to(ostream &out) const {
    out << "node " << setfill('0') << hex << setw(2) << node
        << " has no connection to node " << setw(2) << neighbor;
}

exc_bad_flow::exc_bad_flow(uint32_t new_node, uint32_t new_flow)
    : node(new_node), flow(new_flow) { }
exc_bad_flow::~exc_bad_flow() { }
...
exc_bad_flow_from::~exc_bad_flow_from() { }
...
err_route_not_static::err_route_not_static() { }
err_route_not_static::~err_route_not_static() { }
...
err_route_not_terminated::err_route_not_terminated(uint32_t f, uint32_t n)
    : flow(f), node(n) { }
err_route_not_terminated::~err_route_not_terminated() { }
...
err_duplicate_flow::err_duplicate_flow(uint32_t new_node,
                                       uint32_t new_flow)
    : node(new_node), flow(new_flow) { }
err_duplicate_flow::~err_duplicate_flow() { }
...
err_duplicate_flow_rename::~err_duplicate_flow_rename() { }
...
exc_bad_transmission(uint32_t new_node, uint32_t new_xmit_id)
    : node(new_node), xmit_id(new_xmit_id) { }
exc_bad_transmission::~exc_bad_transmission() { }
...
exc_dma_busy::exc_dma_busy(uint32_t new_node, uint32_t new_dma)
    : node(new_node), dma(new_dma) { }
exc_dma_busy::~exc_dma_busy() { }
void exc_dma_busy::show_to(ostream &out) const {
    out << "DMA channel " << setfill('0') << hex
        << setw(2) << node << ":" << setw(2) << dma << " busy";
}

err_bad_arb_scheme::err_bad_arb_scheme(uint32_t new_arb_scheme)
  : arb_scheme(new_arb_scheme) { }
err_bad_arb_scheme::~err_bad_arb_scheme() { }
...
err_bad_arb_min_bw::err_bad_arb_min_bw(uint32_t new_src, uint32_t new_dst,
    ...
    : src(new_src), dst(new_dst), s2d_bw(new_s2d_bw), d2s_bw(new_d2s_bw),
      min_bw(new_min_bw) { }
err_bad_arb_min_bw::~err_bad_arb_min_bw() { }
...
err_parse::err_parse(const string &f, const string &m)
    : file(f), line(0), msg(m) { }
err_parse::err_parse(const string &f, unsigned l, const string &m)
    : file(f), line(l), msg(m) { }
...
err_bad_shmem_cfg::err_bad_shmem_cfg(const string &new_msg) : msg(new_msg) { }
err_bad_shmem_cfg::err_bad_shmem_cfg(const char *new_msg) : msg(new_msg) { }
err_bad_shmem_cfg::~err_bad_shmem_cfg() { }
