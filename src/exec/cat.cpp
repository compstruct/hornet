...
void catStripe::tick_negative_edge() {
    while (!m_entry_queue.empty() && m_entry_queue.front().available_time <= system_time) {
        ...
        req->m_core = (req->m_maddr.address/m_allocation_unit_in_bytes)%m_num_nodes;
        req->m_status = CAT_DONE;
...
void catStatic::request(shared_ptr<catRequest> req) {
    ...
    uint64_t index = req->m_maddr.address/m_allocation_unit_in_bytes;
    if (m_map[req->m_maddr.mem_space_id].count(index) > 0) {
        req->m_core = m_map[req->m_maddr.mem_space_id][index].core;
        if (m_map[req->m_maddr.mem_space_id][index].available_time > system_time) {
            new_req_entry.available_time = m_map[req->m_maddr.mem_space_id][index].available_time + m_latency;
        } else {
...
void catFirstTouch::request(shared_ptr<catRequest> req) {
    uint32_t space = req->m_maddr.mem_space_id;
    uint64_t index = (req->m_maddr.address)/m_allocation_unit_in_bytes;
    request_entry_t new_req_entry = { req, 0 };
    if (m_map[space].count(index) == 0) {
        map_entry_t new_map_entry = { req->m_sender, system_time + m_synch_delay };
        m_map[space][index] = new_map_entry;
        new_req_entry.available_time = system_time + m_synch_delay + m_latency;
        req->m_core = req->m_sender;
    } else {
        req->m_core = m_map[space][index].core;
