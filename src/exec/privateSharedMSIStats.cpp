...
void privateSharedMSIStatsPerTile::did_invalidate_sharers(uint32_t num_sharers, uint64_t latency) {
    m_invalidated_sharers.add(num_sharers, 1);
...
void privateSharedMSIStats::print_stats(ostream &out) {
    ...
    sprintf(str, "[Summary: Private-shared-MSI ] L1hit%%: %.2f read%%: %.2f write%%: %.2f "
            ...
            total_inv_sharers/total_l2_writes, 
