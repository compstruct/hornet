...
common_core::common_core(const pe_id &id, const uint64_t &t, std::shared_ptr<id_factory<packet_id> > pif,
           std::shared_ptr<tile_statistics> st, logger &l, std::shared_ptr<random_gen> r, 
           std::shared_ptr<memory> mem,
           uint32_t number_of_core_msg_types, uint32_t msg_q_size, uint32_t bytes_per_flit) :
...
{ 
    ...
    for (uint32_t i = 0; i < m_number_of_msg_types; ++i) {
        m_in_msg_queues[i] = std::shared_ptr<messageQueue> (new messageQueue(i, m_msg_queue_size));
        m_out_msg_queues[i] = std::shared_ptr<messageQueue> (new messageQueue(i, m_msg_queue_size));
    }
...
void common_core::connect(std::shared_ptr<bridge> net_bridge) {
    m_net = net_bridge;
    std::shared_ptr<vector<uint32_t> > qs = m_net->get_ingress_queue_ids();
    m_queue_ids.clear();
    copy(qs->begin(), qs->end(), back_insert_iterator<vector<uint32_t> >(m_queue_ids));
}

std::shared_ptr<messageQueue> common_core::receive_queue(uint32_t type) {
    assert((uint32_t)type < m_number_of_msg_types);
    return m_in_msg_queues[type];
}

std::shared_ptr<messageQueue> common_core::send_queue(uint32_t type) {
    assert((uint32_t)type < m_number_of_msg_types);
    return m_out_msg_queues[type];
}

void common_core::release_xmit_buffer() {
...
void common_core::tick_positive_edge() {
    ...
    for (uint32_t it = 0; it < m_number_of_msg_types; ++it) {
        ...
        std::shared_ptr<messageQueue> send_q = m_out_msg_queues[type];
        if (send_q->size() == 0) {
            continue;
        }
    ...
    for (uint32_t it = 0; it < 32 ; ++it) {
        ...
        if (m_incoming_packets.find(channel) != m_incoming_packets.end()) {
            ...
            if (m_net->get_transmission_done(ip.xmit)) {
                ...
                std::shared_ptr<message_t> new_msg = std::shared_ptr<message_t>(new message_t);
                *new_msg = *old_msg;
...
void common_core::tick_negative_edge() {
    tick_negative_edge_memory();
}

void common_core::tick_positive_edge_memory() {
    if (m_memory != std::shared_ptr<memory>()) {
        m_memory->tick_positive_edge();
    }
}
void common_core::tick_negative_edge_memory() {
    /* update requests */
...
/* not used methods */
void common_core::add_packet(uint64_t time, const flow_id &flow, uint32_t len) { assert(false); }
bool common_core::work_queued() { assert(false); return false; }
bool common_core::is_ready_to_offer() { assert(false); return false; }
void common_core::set_stop_darsim() { assert(false); }

uint64_t common_core::next_pkt_time() {
    return system_time;
}
