...
void mcpu::exec_core() {
    //printf("[mcpu %d] Called exec_core {tid: %d, pc: %x}\n", get_id().get_numeric_id(), *((uint32_t *) src), *((uint32_t *) src+1));
        ...
        if (tid == get_id().get_numeric_id()) {
            ...
            if (contexts[1].active == false && 
                ...
                load_context_start(msg.core_msg.context, 1);
                        ...
                        !conservative_pending_request()) { // TODO: stall eviction process for pending memory requests?
                ...
                bool c4 =   contexts[1].active == true &&
                            conservative_pending_request() == true;
                assert(c1 || c2 || c3 || c4);
                if (DEBUG_EM2) {
...
#endif
        data_complete();
        shared_ptr<instr> i = instruction_fetch_complete(get_context()->pc);    
        if (running_array[get_context()->tid] && i && !pending_request()) {
            instr_count++;
            execute(i);
            if (!pending_request()) {
...
shared_ptr<instr> mcpu::instruction_fetch_complete(uint32_t pc) {
    if (!enable_memory_hierarchy) {
        ...
        if (get_context()->pending_i_request && !pending_request() && 
            nearest_i_memory()->ready(get_context()->pending_i_reqid)) {
            shared_ptr<memoryRequest> ld_req = 
                        nearest_i_memory()->get_req(get_context()->pending_i_reqid);
        ...
        if (!get_context()->pending_i_request) {
            //if (get_id() == 0)
                //printf("[mcpu %d, %d] Issued instruction_fetch_complete, address: %x\n", get_id().get_numeric_id(), current_context, pc);
            shared_ptr<memoryRequest> read_req(new memoryRequest(
                                get_context()->tid, get_context()->pc, BYTE_COUNT));
...
bool mcpu::data_fetch_read( const uint32_t &addr, 
    ...
    //printf("[mcpu %d, %d] Memory read request at %x\n", get_id().get_numeric_id(), current_context, addr);
    get_context()->pending_req = shared_ptr<memoryRequest> (
...
bool mcpu::data_fetch_write(    const uint32_t &addr, 
                                ...
                                const uint32_t &bytes) throw(err) {
    assert(!pending_request());
    //printf("[mcpu %d, %d] Memory write request at %x\n", get_id().get_numeric_id(), current_context, addr);
...
void mcpu::execute(shared_ptr<instr> ip) throw(err) {
    ...
    case IC_BC1F: unimplemented_instr(i, get_context()->pc);
    case IC_BC1FL: unimplemented_instr(i, get_context()->pc);
    case IC_BC1T: // FP
        if (get_cp1_cf(cfr(0))) branch(); 
        break;
    case IC_BC1TL: unimplemented_instr(i, get_context()->pc);
    ...
    case IC_INS: set(i.get_rt(), splice(get(i.get_rt()), get(i.get_rs()),
                                        i.get_msb(), i.get_lsb()));
                 break;
    case IC_J: jump(((get_context()->pc + 4) & 0xf0000000) | (i.get_j_tgt() << 2)); break;
    case IC_JAL: set(gpr(31), get_context()->pc + 8);
                 jump(((get_context()->pc + 4) & 0xf0000000) | (i.get_j_tgt() << 2));
                 break;
    case IC_JALR:
    case IC_JALR_HB: set(i.get_rd(), get_context()->pc + 8); jump(get(i.get_rs())); break;
    case IC_JR:
    case IC_JR_HB: {
