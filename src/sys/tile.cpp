...
tile::tile(const tile_id &new_id, const uint32_t num_tiles,
    ...
    pid_factory =
        std::shared_ptr<id_factory<packet_id> >(new id_factory<packet_id>(templ,
                                                                     mask));
}

void tile::add(std::shared_ptr<pe> p) {
...
void tile::add(std::shared_ptr<bridge> b) {
...
void tile::add(std::shared_ptr<node> n) {
...
void tile::add(std::shared_ptr<arbiter> a) {
...
const uint64_t &tile::get_time() const {
    return time;
}

std::shared_ptr<tile_statistics> tile::get_statistics() const {
    return stats;
}

std::shared_ptr<id_factory<packet_id> > tile::get_packet_id_factory() const {
    return pid_factory;
}
...
void tile::fast_forward_time(uint64_t new_time) {
    assert(new_time >= time);
    assert(is_drained());
    time = new_time;
}

bool tile::is_ready_to_offer() const {
    for (pes_t::const_iterator i = pes.begin(); i != pes.end(); ++i) {
...
uint64_t tile::next_pkt_time() const {
