...
void ginj::connect(std::shared_ptr<bridge> net_bridge) {
    net = net_bridge;
...
void ginj::add_packet(uint64_t time, const flow_id &f, unsigned l) {
    if (!stop_darsim) {
...
void ginj::set_stop_darsim() {
   LOG(log,0) << "Stop [ginj " << get_id() << "]" << endl;
   stop_darsim = true;
}

bool ginj::work_queued() {
    if (stop_darsim) {
       return 0;
    }
...
void ginj::tick_negative_edge() { }

bool ginj::is_ready_to_offer() {
   if (stop_darsim) {
      return false;
   }
   queue<g_waiting_packet> &q = waiting_packets_queue;
