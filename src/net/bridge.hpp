...
class bridge {
public:
    explicit bridge(std::shared_ptr<node> n,
    ...
    uint32_t get_transmission_done(uint32_t transmission);
    uint32_t get_waiting_queues();
    ...
    std::shared_ptr<egress> get_egress();
    std::shared_ptr<ingress> get_ingress();
    std::shared_ptr<vector<uint32_t> > get_ingress_queue_ids();
private:
    const std::shared_ptr<ingress_dma_channel> &get_ingress_dma(uint32_t) const
       ;
    const std::shared_ptr<egress_dma_channel> &get_egress_dma(uint32_t) const
       ;
private:
    typedef map<virtual_queue_id, std::shared_ptr<ingress_dma_channel> >
        ingress_dmas_t;
    typedef map<virtual_queue_id, std::shared_ptr<egress_dma_channel> >
        egress_dmas_t;

    const node_id id;
    std::shared_ptr<node> target;
    std::shared_ptr<bridge_channel_alloc> vc_alloc;
    std::shared_ptr<ingress> incoming;
    std::shared_ptr<egress> outgoing;
...
inline const node_id &bridge::get_id() const { return id; }
